...
double computeOmega(IntrinsicGeometryInterface& geometry,
                    const VertexData<Vector2>& directionField,
                    const VertexData<double>& frequencies,
                    const Edge& e,
                    bool* crossesSheets = nullptr)
{
...
extractCrossingsFromStripePattern(EmbeddedGeometryInterface& geometry,
                                  const CornerData<double>& stripeValues,
                                  const FaceData<int>& stripeIndices,
...
std::vector<std::vector<size_t>> removeCrossings(std::vector<std::pair<double, std::array<size_t, 2>>>& values,
                                                 int N,
...
double
findMin(const std::vector<std::vector<Vector3>>& points, const Vector3& v, const Vector3& vector, size_t ignoredEdge)
{
 ...
 */
void connectIsolinesOnSingularities(EmbeddedGeometryInterface& geometry,
                                    const CornerData<double>& stripeValues,
                                    const FaceData<int>& stripeIndices,
                                    const FaceData<int>& branchIndices,
                                    EdgeData<std::vector<size_t>>& polylineIndices,
                                    const FaceData<Vector3>& vectorField,
                                    std::vector<Vector3>& vertices,
                                    std::vector<std::array<size_t, 2>>& edges)
{
  SurfaceMesh& mesh = geometry.mesh;
  ...
  for(Face f: mesh.faces())
  {
    if(stripeIndices[f] != 0)
    {
      ...
      for(Halfedge he: f.adjacentHalfedges())
      {
        ...
        if(polylineIndices[he.edge()].size() > 0)
        { // use the existing indices in reverse order
        ...
        { // add the intersections to the vertices lists and initialize the corresponding list of indices
...
extractPolylinesFromStripePattern(EmbeddedGeometryInterface& geometry,
                                  const CornerData<double>& values,
                                  const FaceData<int>& stripeIndices,
                                  const FaceData<int>& fieldIndices,
                                  const VertexData<Vector2>& directionField,
